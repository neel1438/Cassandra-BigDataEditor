...
from tempita import HTMLTemplate	# here using tempita for templating its just a templating language for text subsitution


# the frontend template will come over here
# note :: this the view template that the user will see
# this is just the sample that I had used for testing my code
...
# note :: this is the template when the user clicks on edit or query option
# this is just the sample that I had used for testing my code
...
class BrowserToolApp(object):
	...
	# mention the fields in this function
...
class Page(object):
    ...
    @property
    def mtime(self):		# for last time modified by the user
        if not self.exists:
            return None
        else:
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '-p', '--port',
        default = '8080',		# running on default port
        dest = 'port',
        type = 'int',
        help = 'Port to serve on (default 8080)')
    parser.add_option(
        '--Browser-Tool-App-data',
        default = './BrowserTool',
        dest = 'BrowserTool_data',
        help = 'Place to put Browser Tool data into (default ./BrowserTool/)')

    options, args = parser.parse_args()
    print 'Writing our Broswer Tool application pages to %s' % options.BrowserTool_data
    app = BrowserToolApp(options.BrowserTool_data)
